...
async function initialize(): Promise<void> {
  try {
    ...
    console.error('[Armor of God] Content script initialization failed:', error);
  }
}
...
async function scanImage(img: HTMLImageElement): Promise<void> {
  try {
    ...
    console.log('[Armor of God] Scanning image:', context.src.substring(0, 100));
...
function blurImage(img: HTMLImageElement, result: ScanResult): void {
  ...
  img.addEventListener('click', (e) => handleBlurredImageClick(e, img, result), { once: false });
...
function handleBlurredImageClick(e: Event, img: HTMLImageElement, result: ScanResult): void {
  e.preventDefault();
  e.stopPropagation();

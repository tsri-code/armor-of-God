...
export async function updateSafeSearchRules(
  enabled: boolean,
  customRules?: any[]
): Promise<void> {
  try {
...
function handleSearchRequest(details: any): any {
  try {
    ...
    if (url.hostname === "www.bing.com" && url.pathname === "/search") {
      if (
        !url.searchParams.has("adlt") ||
        url.searchParams.get("adlt") !== "strict"
      ) {
    ...
    if (url.hostname === "duckduckgo.com" && url.pathname === "/") {
      if (
        !url.searchParams.has("safe-search") ||
        url.searchParams.get("safe-search") !== "strict"
      ) {
    ...
    if (url.hostname === "www.youtube.com") {
      if (
        !url.searchParams.has("restrict_mode") ||
        url.searchParams.get("restrict_mode") !== "1"
      ) {
        url.searchParams.set("restrict_mode", "1");
        needsRedirect = true;
      }
...
export async function testSafeSearchStatus(
  url: string
): Promise<{ active: boolean; provider?: string }> {
  try {
...
export async function getSafeSearchStats(): Promise<any> {
  try {
    ...
    const safeSearchRules = rules.filter(
      (rule) =>
        rule.id >= CONSTANTS.RULES.SAFE_SEARCH_RULE_ID_START &&
        rule.id < CONSTANTS.RULES.USER_RULE_ID_START
    );

...
browser.runtime.onInstalled.addListener(async (details) => {
  try {
    logger.info('service-worker', 'Extension installed', { reason: details.reason });
...
async function handleFirstInstall(): Promise<void> {
  ...
  logger.info('service-worker', 'Default settings initialized', { enabled: settings.enabled });
...
async function initializeExtension(): Promise<void> {
  ...
  try {
    // Load and apply blocking rules
    await loadInitialRules();
    await applyUserRules();
    
    // Setup safe search enforcement
    await enforceSafeSearch();
    
    // Setup scheduled tasks
    await setupAlarms();
    
    // Fetch verse of the day if needed
...
browser.storage.onChanged.addListener(async (changes, areaName) => {
  ...
  try {
    ...
    if (changes.settings) {
      ...
      if (newSettings?.modules?.urlBlocking !== oldSettings?.modules?.urlBlocking ||
      ...
      if (newSettings?.modules?.safeSearch !== oldSettings?.modules?.safeSearch) {
        await updateSafeSearchRules(newSettings.modules.safeSearch);
      }
      
      // Update alarms if schedules changed
      if (JSON.stringify(newSettings?.schedules) !== JSON.stringify(oldSettings?.schedules)) {
        await setupAlarms();
      }
    }
  } catch (error) {
...
browser.alarms.onAlarm.addListener(async (alarm) => {
  try {
    ...
    logger.error('service-worker', 'Alarm handler failed', { alarmName: alarm.name }, error);
  }
});
...
browser.runtime.onMessage.addListener(async (message, sender, sendResponse) => {
  try {
    logger.debug('service-worker', 'Message received', { type: message.type, from: sender.tab?.id });
    ...
    logger.error('service-worker', 'Message handling failed', { type: message.type }, error);
...
async function handleMessage(message: any, sender: any): Promise<any> {
  switch (message.type) {
    ...
    default:
      logger.warn('service-worker', 'Unknown message type', { type: message.type });
...
async function toggleExtension(): Promise<any> {
  try {
    ...
    logger.info('service-worker', `Extension ${newEnabled ? 'enabled' : 'disabled'}`);

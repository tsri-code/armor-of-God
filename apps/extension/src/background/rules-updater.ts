...
export async function loadInitialRules(): Promise<void> {
  try {
    if (!supportsDeclarativeNetRequest()) {
      logger.info('rules-updater', 'DNR not supported, will use webRequest fallback');
      return;
    }

    logger.info('rules-updater', 'Loading initial blocking rules', { count: initialRules.length });
    
    // Initial rules are loaded via manifest.json rule_resources
    // But we can verify they're active
    const staticRules = await browser.declarativeNetRequest.getStaticRules();
    logger.info('rules-updater', 'Static rules loaded', { count: staticRules.length });
...
export async function applyUserRules(): Promise<void> {
  try {
    ...
    logger.info('rules-updater', 'User rules applied', { count: userRules.length });
...
export async function clearUserRules(): Promise<void> {
  try {
    ...
    if (userRuleIds.length > 0) {
      ...
      logger.info('rules-updater', 'Cleared user rules', { count: userRuleIds.length });
    }
  } catch (error) {
...
export async function testUrlAgainstRules(url: string): Promise<{ blocked: boolean; reason?: string }> {
  try {
    const settings = await storage.getSettings();
    ...
    for (const rule of initialRules) {
      if (rule.condition.urlFilter) {
        const pattern = rule.condition.urlFilter.replace(/\|\|([^\/\^]+)\^/, '$1');
...
// Get rule statistics
export async function getRuleStats(): Promise<{ static: number; dynamic: number; user: number }> {
  try {
    if (!supportsDeclarativeNetRequest()) {
    ...
    const [staticRules, dynamicRules] = await Promise.all([
      browser.declarativeNetRequest.getStaticRules(),
      browser.declarativeNetRequest.getDynamicRules(),
    ]);
    
    const userRules = dynamicRules.filter(rule => 
...
export async function importRules(data: any): Promise<void> {
  try {
    ...
    const updatedSettings = {
      ...settings,
      whitelist: Array.isArray(data.whitelist) ? data.whitelist : settings.whitelist,
      blacklist: Array.isArray(data.blacklist) ? data.blacklist : settings.blacklist,
    };
